...
import { CorrelationResult, GroupComparisonResult, GrowthCurveResult } from '../../types';
...
export const AgingCurveChart: React.FC<AgingCurveProps> = ({ data, config }) => {
    ...
    const overlayData = useMemo(() => {
         ...
         return data.overlay.age.map((a, i) => ({ age: a, value: data.overlay!.values[i] }));
    }, [data.overlay]);
        ...
        <div className="flex gap-4 mb-4 text-xs">
            ...
            {data.overlay && (
                <div className="flex-1 bg-slate-800 p-2 rounded">
                    ...
                    <div className="font-mono text-rose-400">{data.overlay.age.length}</div>
                </div>
            )}
        </div>
