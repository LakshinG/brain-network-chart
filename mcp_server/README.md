...
| MCP tool | `run_medsam_segmentation` | Segment organs in NIfTI scans with embedded MedSAM agent |
| GET | `/medsam_outputs/{path}` | Serve MedSAM overlay/mask outputs generated by the MCP tool |
...
#### MCP tool: `run_medsam_segmentation`

Runs the embedded MedSAM agent on a NIfTI volume. This is exposed through MCP
tool calling rather than a direct REST endpoint.

Required real-inference environment:

```bash
export MEDSAM_CHECKPOINT=/path/to/medsam_vit_b.pth
export SEGMENT_ANYTHING_PATH=/path/to/MedSAM_source
export MEDSAM_DEVICE=cuda
```

Optional:

```bash
export ORGAN_DETECTOR_CHECKPOINT=/path/to/organ_bbox_unet.pth
export MEDSAM_OUTPUT_DIR=/path/to/medsam_outputs
```
...
For a no-checkpoint smoke test, use `"backend": "mock"` and `"device": "cpu"`.
The tool returns `mask_path`, `overlay_path`, `overlay_url`, `voxel_count`,
`seeds_used`, and `box_used`.
