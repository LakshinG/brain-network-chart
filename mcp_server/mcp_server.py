...
from medsam_agent.service import (
    resolve_output_file,
    run_segmentation as run_medsam_pipeline,
)
...
@server.tool(name="run_medsam_segmentation", description="""
Segment an organ or tumor in a local/uploaded NIfTI scan using the embedded
MedSAM agent pipeline. Use this when the user asks to segment, outline, or
delineate an organ in a .nii or .nii.gz scan.

Parameters:
- scan_path: Absolute NIfTI path, or an uploaded file name/path resolvable by the MCP server.
- organ: Natural language organ name, e.g. right kidney, left kidney, liver, spleen, heart, myocardium.
- backend: medsam (default), medsam2, sam, or mock. Use mock only for no-checkpoint smoke tests.
- output_dir: Optional output root. Defaults to MEDSAM_OUTPUT_DIR or ./medsam_outputs.

Requires environment variables for real inference:
MEDSAM_CHECKPOINT, SEGMENT_ANYTHING_PATH, and optionally ORGAN_DETECTOR_CHECKPOINT and MEDSAM_DEVICE.
""")
def run_medsam_segmentation(
    scan_path: str,
    organ: str,
    ...
    """
    Run MedSAM organ segmentation and return output file paths plus summary metadata.
    """
    start_time = time.time()
    progress_log = []
    try:
        ...
        progress_log.append({"step": "resolve_scan", "message": f"Resolving scan path: {scan_path}"})
        resolved_scan = get_file_path(scan_path, uploaded_only=False)

        progress_log.append({"step": "segment", "message": f"Running MedSAM segmentation for organ={organ}"})
        result = run_medsam_pipeline(
            resolved_scan,
            organ,
        ...
        elapsed = time.time() - start_time
        progress_log.append({"step": "done", "message": f"Segmentation complete in {elapsed:.2f}s"})
        result.update({
            "timestamp": datetime.now().isoformat(),
            "elapsed_seconds": elapsed,
            "progress": progress_log,
        ...
        elapsed = time.time() - start_time
        logger.error(f"MedSAM segmentation error: {str(e)}", exc_info=True)
...
@server.custom_route("/medsam_outputs/{path:path}", methods=["GET"])
async def medsam_output_file(request: Request) -> FileResponse:
    """Serve MedSAM output artifacts generated by run_medsam_segmentation."""
    relative_path = request.path_params.get("path", "")
    try:
        return FileResponse(resolve_output_file(relative_path))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail="MedSAM output file not found")
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid MedSAM output path")
