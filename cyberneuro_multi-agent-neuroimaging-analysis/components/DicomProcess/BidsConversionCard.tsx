import type { BidsConversionResult } from './BidsConversionForm'
import { useState, useEffect, useRef } from 'react'
import { getBackendBaseUrl } from '../../services/mcpService'
...
export const DATA_HOST_URL = 'http://localhost:7789'
...
export default function BidsConversionCard({ data, timestamp, onComplete }: Props) {
  ...
  const [pipelineDir] = useState('/ram/USERS/tao/code/new/bids')
  const [processDir] = useState('process_output')
  const [scFcDir] = useState('sc_fc_output')
    ...
    async function start() {
      ...
      if (runMode === 'local') {
        try {
          ...
          const hr = await fetch(`${DATA_HOST_URL}/health`, { signal: ctrl.signal })
          clearTimeout(t)
          if (hr.ok && !cancelled) {
            ...
            const runRes = await fetch(`${DATA_HOST_URL}/run`, {
              method: 'POST',
            ...
            if (cancelled) return
            es = new EventSource(`${DATA_HOST_URL}/stream/${job_id}`)
            ...
            es.addEventListener('done', async (e) => {
              ...
              try {
                const rr = await fetch(
                  `${DATA_HOST_URL}/read_file?path=${encodeURIComponent(data.output_dir + '/conversion_report.html')}`
                )
  ...
  useEffect(() => {
    ...
    async function checkHealth() {
      try {
        ...
        const r = await fetch(`${DATA_HOST_URL}/health`, { signal: controller.signal })
        clearTimeout(timer)
  ...
  async function handleRun(step: Step) {
    ...
    try {
      ...
      if (postProcMode === 'local') {
        ...
        es = new EventSource(`${getBackendBaseUrl()}/run_command_stream?${params}`)
        es.onmessage = (e) => onLine(e.data)
        es.addEventListener('done', (e) => {
          const { exit_code } = JSON.parse((e as MessageEvent).data)
          onDone(exit_code); es.close()
